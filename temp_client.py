...
#     try:
#         client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
#         client_socket.connect(('localhost', 1709))
#         client_socket.setblocking(1)  # Temporarily set to blocking mode to receive initial color message
#         return client_socket
#     except Exception as e:
#         print(f"Error setting up network: {e}")
...
#         if new_socket:
#             try:
#                 encrypted_data = new_socket.recv(1024)
#                 if len(encrypted_data) < 16:
#                     print("Received invalid encrypted data, ignoring.")
#                     continue
#                 ciphertext, tag = encrypted_data[:-16], encrypted_data[-16:]  # Assuming the tag is 16 bytes long
#                 new_color = cipher.aes_decryption(ciphertext, tag)
...
#         print(f"want reset: {want_reset}")
#         if want_reset:
#             move_msg = create_move_msg("reset", (-1, -1), (-1, -1))
#             encrypted_msg, tag = cipher.aes_encryption(move_msg)
#             client_socket.sendall(encrypted_msg + tag)
...
#     :param game_running_ref: A reference to the game running state (list with one boolean element).
#     """
#     global opponent_disconnected
#     try:
#         while game_running_ref[0] and is_socket_open(client_socket) and not game.should_quit and not opponent_disconnected:
#             try:
#                 if not isinstance(client_socket, socket.socket):
#                     print("Invalid socket detected.")
#                     break
...
#                 if move:
#                     print(f"Received move from server: {move}")
#                     move_queue.put(move)
#             except BlockingIOError:
#                 pass  # Just pass on BlockingIOError to try receiving again
#             except ConnectionResetError as e:
...
#                 return
#             ciphertext, tag = encrypted_data[:-16], encrypted_data[-16:]  # Assuming the tag is 16 bytes long
#             my_color = cipher.aes_decryption(ciphertext, tag)
#             print(f"Assigned color: {my_color}")
#         except Exception as e:
#             print(f"Error receiving color assignment: {e}")
#             if client_socket:
...
#                 color_won = this_turn_player
#         if game.move_type is not None and game.my_color == game.this_turn_color:
#             if game.chosen_piece_pos == game.last_move_to and game.chosen_piece_pos != (-1, -1):
#                 game.execute_move(game.chosen_piece_pos, this_turn_player, other_turn_player)
#             move_msg = create_move_msg(game.move_type, game.chosen_piece_pos, game.last_move_to)
